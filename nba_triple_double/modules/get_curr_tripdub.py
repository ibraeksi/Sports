...
def get_curr_tripdub(latest_date, latest_num_td):
    """
    Retrieves current season triple-doubles using nba_api and returns a table
    with game logs in the required format
    latest_date = Most recent date when the current season data was updated
    latest_num_td = Number of triple-doubles in the most recent current season data
    """
    ...
    if tripdubdf['TD3'].sum() != latest_num_td:
        ...
        for id in list(playeriddict.keys()):
            ...
            temp['DATE'] = pd.to_datetime(temp['GAME_DATE']).dt.date.astype(str)
            # Filter for games with triple-doubles
            temptd = temp[(temp['DATE'] > latest_date) & (temp['PTS'] >= 10) & (temp['REB'] >= 10) & ((temp['AST'] >= 10) | (temp['STL'] >= 10) | (temp['BLK'] >= 10))].reset_index(drop=True)
